...
def get_pull_request_info(event_file_path):
    ...
    res = {
        ...
        "url": f"https://github.com/Sidney98204/test-actions2/pull/{event_obj['pull_request']['number']}"
    }
    return res
...
def check_issue_exist_already(team_id, pr_url, api_token):
    headers = {
...
def fetch_workflow_states(team_id, api_token):
    headers = {
        'Content-Type': 'application/json',
        'Authorization': api_token,
    }
...
def search_jira_for_issue(team_id, pr_url, auth):

   search_issue_url = "https://sids-test-env.atlassian.net" + "/rest/api/3/search"
...
def create_jira_issue(title, description, team_id, assignee_id, auth):
   issue_url = "https://sids-test-env.atlassian.net" + "/rest/api/3/issue"
...
def transition_jira_issue(issue_key, auth):
   transitions_url = "https://sids-test-env.atlassian.net" + f"/rest/api/3/issue/{issue_key}/transitions"
